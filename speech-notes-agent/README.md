...
1. **observe** — `AudioScannerTool` сканирует папку `AUDIO_DIR` на новые файлы (`.mp3`, `.ogg`, `.wav`). Уже обработанные файлы отфильтровываются через память.
...
- `AUDIO_EXTENSIONS` — список расширений через запятую (по умолчанию `.mp3,.ogg,.wav`; `.m4a` не поддерживается SpeechKit STT v1 без предварительной перекодировки).

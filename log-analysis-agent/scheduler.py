"""
scheduler.py — планировщик агента анализа логов
Запускает агента каждые N часов (по умолчанию 1) — IntervalTrigger,
в отличие от агентов с фиксированным временем запуска.
"""
...
def start():
    ...
    scheduler.add_job(
        ...
        id="log_analysis",
        name="Анализ логов Yandex Cloud Logging",
        misfire_grace_time=600,
        coalesce=True,
    )

    logger.info(
        f"Планировщик запущен. Агент будет работать каждые "
        f"{config.check_interval_hours} ч. ({config.timezone})."
    )
